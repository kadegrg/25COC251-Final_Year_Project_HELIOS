...
import { useWriteOffStock } from '@/hooks/use-stock-movements';
...
import type { WriteOffRequest } from '@/types/stock-movements.types';

export function WriteOffStockPage() {
  const mutation = useWriteOffStock();
  const [form, setForm] = useState<WriteOffRequest>({ siteId: '', locationId: '', skuId: '', quantity: 1 });
  const [successId, setSuccessId] = useState<string>();
  const set = (field: string, value: string | number) => setForm((f) => ({ ...f, [field]: value }));

  return (
    <StockActionLayout
      title="Write Off Stock" description="Write off stock (requires MFA / AAL 2)" breadcrumbLabel="Write Off"
      isPending={mutation.isPending} error={mutation.error} successMovementId={successId}
      requiredPermission={STOCK_PERMISSIONS.STOCK_ADJUST}
    ...
    >
      ...
      <FormField label="From Status Code">
        <select className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm" value={form.fromStatusCode ?? 'DAMAGED'} onChange={(e) => set('fromStatusCode', e.target.value)}>
          {STOCK_STATUS_CODES.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
      </FormField>
      <FormField label="Reason Code"><Input value={form.reasonCode ?? 'WRITE_OFF'} onChange={(e) => set('reasonCode', e.target.value)} /></FormField>
