...
import { useStatusChange } from '@/hooks/use-stock-movements';
...
import type { StatusChangeRequest } from '@/types/stock-movements.types';

export function StatusChangePage() {
  const mutation = useStatusChange();
  const [form, setForm] = useState<StatusChangeRequest>({
    siteId: '', locationId: '', skuId: '', quantity: 1, fromStatusCode: 'SELLABLE', toStatusCode: 'DAMAGED',
  });
  ...
  return (
    <StockActionLayout
      title="Status Change" description="Change stock status code" breadcrumbLabel="Status Change"
    ...
    >
      ...
      <FormField label="From Status Code" required>
        <select className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm" value={form.fromStatusCode} onChange={(e) => set('fromStatusCode', e.target.value)}>
          {STOCK_STATUS_CODES.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
      </FormField>
      <FormField label="To Status Code" required>
        <select className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm" value={form.toStatusCode} onChange={(e) => set('toStatusCode', e.target.value)}>
          {STOCK_STATUS_CODES.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
      </FormField>
