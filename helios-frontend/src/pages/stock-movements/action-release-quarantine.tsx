...
import { useReleaseQuarantine } from '@/hooks/use-stock-movements';
...
import type { ReleaseQuarantineRequest } from '@/types/stock-movements.types';

export function ReleaseQuarantinePage() {
  const mutation = useReleaseQuarantine();
  const [form, setForm] = useState<ReleaseQuarantineRequest>({ siteId: '', locationId: '', skuId: '', quantity: 1 });
  const [successId, setSuccessId] = useState<string>();
  const set = (field: string, value: string | number) => setForm((f) => ({ ...f, [field]: value }));

  return (
    <StockActionLayout
      title="Release Quarantine" description="Release stock from quarantine" breadcrumbLabel="Release Quarantine"
    ...
    >
      ...
      <FormField label="To Status Code">
        <select className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm" value={form.toStatusCode ?? 'SELLABLE'} onChange={(e) => set('toStatusCode', e.target.value)}>
